...
router.get('/guest-info/:slug', async (req, res) => {
  // Этим же запросом приложение проверяет, жив ли вход, и кэшировать его
  // нельзя. Раньше ответ уходил с ETag и без Cache-Control: при следующем
  // холодном запуске телефон переспрашивал с If-None-Match, главный вход
  // честно отвечал 304, а проверка живости считала 304 за «не ответил» и
  // уходила на запасной вход — тот, что у российских операторов закрыт.
  // Отсюда «первый раз открывается, а после полного закрытия — только
  // через VPN»: пока приложение не закрыто, проверка не повторяется.
  //
  // Заголовки условного запроса снимаем: без них Express не может ответить
  // 304 и всегда отдаёт 200. Плюс запрещаем кэшировать ответ впредь. Это
  // лечит и те сборки, что уже стоят у людей, — обновление не требуется.
