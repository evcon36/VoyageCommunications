...
// Никаких ETag на ответах API.
//
// Express по умолчанию считает ETag для каждого JSON-ответа, а Cache-Control
// не ставит. Из-за этого телефон при следующем холодном запуске переспрашивал
// «не изменилось?» и получал 304 — законный ответ, но обёртка запросов в
// клиенте считает успехом только 200-ю серию, а у 304 ещё и тела нет. Выходило
// «Нет связи с сервером» при живом сервере, причём ровно со второго запуска:
// первый клал ответ в кэш, второй на нём и спотыкался. Под VPN не проявлялось,
// потому что запасной вход через Cloudflare отдаёт 200 и успевал ответить
// первым.
//
// Экономия трафика от 304 на ответах в пару килобайт не стоит ничего, а
// ломала вход в приложение целиком.
app.set('etag', false);
