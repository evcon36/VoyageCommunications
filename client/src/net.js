...
const CANDIDATES = (() => {
  ...
  // В приложении на телефоне запасных входов нет — только апекс.
  //
  // Запасной вход живёт за Cloudflare, а тот в каждом ответе объявляет
  // `alt-svc: h3=":443"`, то есть просит ходить к нему по HTTP/3 (UDP) целые
  // сутки. Мобильные операторы UDP/443 режут, и телефон на каждом запросе
  // сначала молча ждёт QUIC, потом откатывается на TCP. В логах это видно как
  // семь секунд задержки; приложение к тому моменту уже сдавалось.
  //
  // Отсюда вся картина: первый запуск шёл по TCP и работал, дальше телефон
  // помнил alt-svc и лез по UDP — и «после закрытия открывается только через
  // VPN». На Wi-Fi UDP не режут, поэтому там работало всегда.
  //
  // Апекс за Cloudflare не стоит, alt-svc не отдаёт и на мобильном интернете
  // единственный доходит стабильно (см. блок по SNI). Цена решения: если
  // апекс ляжет, у приложения не останется запасного адреса. В браузере
  // запасные входы остаются — там QUIC не мешает, а польза от них есть.
  const inApp = Boolean(globalThis.Capacitor?.isNativePlatform?.());
  if (isProd && !inApp) {
