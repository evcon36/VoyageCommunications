  ...
  if (canGoNative(init)) {
    try {
      ...
      const r = await nativeHttp.request({
        ...
        // Попытка очень короткая, попыток много.
        //
        // Удачное рукопожатие на этой сети занимает доли секунды — по записи
        // пакетов от 0,08 до 0,5 с. А неудачное не завершится никогда: у него
        // теряется хвост приветствия, и ждать его бессмысленно. Значит, весь
        // смысл в том, чтобы быстрее начать следующую попытку.
        //
        // Замер на сборке 121: при двух секундах на попытку запуск занимал
        // 11 секунд, то есть проходила примерно шестая попытка.
        timeout: 1.2,
        attempts: Math.max(6, Math.min(30, Math.round(timeoutMs / 900))),
      });
