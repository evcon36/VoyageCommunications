...
function startSocket() {
  ...
  socketStarted = true;
  // Без предварительной проверки входа: она занимала бы единственное
  // соединение, а сокету нужен адрес, который уже выбран запросами.
  try { socket.io.uri = serverUrl(); socket.io.opts.host = undefined; } catch { /* адрес не сменился */ }
  socket.connect();
}
...
// связи с сервером» до полного перезапуска приложения. Поэтому на неудачное
...
export default function App() {
  ...
  const checkAuth = useCallback(async () => {
    ...
    // Проверки входов при запуске нет вовсе.
    //
    // Сначала она стояла ПЕРЕД входом и задерживала его на 15 секунд. Я увёл
    // её в параллель — стало 10, но не ушло: приложению разрешено одно
    // соединение на адрес (это защита от лавины одновременных рукопожатий),
    // и проверка просто занимала его, пока вход стоял в очереди. Замер:
    //
    //     12:13:11  запуск
    //     12:13:20  проверка входа наконец доходит
    //     12:13:23  вход — двенадцатью секундами позже запуска
    //
    // Проверять заранее незачем: вход в приложении один, а если он не
    // ответит, перебор входов внутри apiFetch сам найдёт рабочий. Проверка
    // осталась только там, где она к месту — когда сокет не смог
    // подключиться (см. connect_error).
