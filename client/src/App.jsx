...
export default function App() {
  ...
  const checkAuth = useCallback(async () => {
    ...
    // Выбор входа больше НЕ задерживает запуск.
    //
    // Раньше приложение сначала проверяло входы и только потом входило в
    // аккаунт. На мобильном интернете проверка перебирала попытки и упиралась
    // в предел в 15 секунд — ровно те двадцать секунд, за которые приложение
    // «открывалось». При этом сам вход после неё занимал три секунды.
    //
    // Проверять заранее незачем: первый же настоящий запрос сам выберет
    // рабочий вход, а перебор входов внутри apiFetch никуда не делся. Пусть
    // проверка идёт рядом и уточняет выбор к следующему запросу.
    pickOrigin().catch(() => { /* не вышло — запросы разберутся сами */ });
