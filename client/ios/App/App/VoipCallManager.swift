...
final class VoipCallManager: NSObject {
    ...
    private func makeSession() -> URLSession {
        ...
        // пяти. При этом Safari на том же телефоне и к тому же серверу
        // работает всегда — а он открывает одно соединение, а не десяток
        // разом. Похоже, оператор гасит не рукопожатие само по себе, а пачку
        // одновременных. Поэтому ведём себя как Safari: одно соединение, по
        // нему все запросы подряд. Медленнее на быстрой сети — незаметно, на
        // плохой это разница между «работает» и «нет связи».
